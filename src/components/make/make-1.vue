<template>
  <div class="make">
    ...
    <div class="content">
      ...
      <div class="mount">
        <span class="mark">&yen;</span>{{ mount }}
      </div>
    </div>

    <div class="key-board" @click.stop.prevent="enter">
      <div class="line">
        <div class="key">
      ...
      <div class="line">
        ...
        <div class="key">
          <span class="num" data-num="X">&larr;</span>
        </div>
      </div>
    </div>
    <div class="pay-type">
  ...
  export default {
    ...
    methods: {
      ...
      toScaning: function (scanType, busTyp) {
        let that = this;
        if (!this.$trim(that.mount) && scanType == '02') {
          this.$quickTip('请输入支付金额')
          return;
        }
        window.cordova.exec(function (data) {
          console.log('调用成功', data); //130230378299982080
        ...
        }, 'TPScanCode', 'scanCodeAction', [that.mount]);
      },
      payQrCode: function () {
        let merch = JSON.parse(localStorage.getItem('merchUser'));
        ...
        this.$post('qrcodeRequest', {
          ...
          if (resData.data.returnSts == 'RPM00000') {
            ...
            data.title = payType == 1 ? '光大信用卡支付' : payType == 2 ? '微信支付' : payType == 3 ? '支付宝支付' : payType == 4 ?
              'QQ支付' : '';
            //that.$router.push({name:'main.ewm',params:data});
          } else {
            that.$quickTip(resData.data.returnMsg);
          }
        }, function (errorData) {
