		...
		methods: {
			confirm() {
				...
				if(cDate>nowdate) {
					...
					let vm = this;
					Array.from(this.$refs.lists.querySelectorAll('.active')).forEach((n,i)=>{
					...
					vm.$nextTick(() => {
						...
						vm.$nextTick(() => {
							...
							Array.from(this.$refs.lists.querySelectorAll('.date-item')).forEach((n,i) => {
								let val = cancelValue[i];
								Array.from(n.querySelectorAll('.item')).forEach((j,k)=>{
									if(j.innerHTML==val){
			...
			createdTouchAction(el,index,vm){
				...
				if(el.querySelector('.active')){
					Array.from(el.querySelectorAll('.item')).forEach((n,i) => {
			...
			showDate() {
				if(!this.hasCreate){
					this.$nextTick(() => {
						...
						let vm =this;
						Array.from(lists).forEach(function(n,i){
							let value;
							if(i==0){
